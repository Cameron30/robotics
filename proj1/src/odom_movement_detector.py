...
class MovementDetector(object):
	def __init__(self):
		self._mved_destance = Float64()
		self._mved_destance.data = 0.0
	...
	def odom_callback(self, msg):
		...
		self._mved_destance.data += self.calculate_distance(NewPosition, self._current_position)
		self.updatecurrent_position(NewPosition)
		if self._mved_destance < 0.000001:
			aux = Float64()
			aux.data = 0.0
			self.distance_moved_pub.publish(aux)
		else:
			self.distance_moved_pub.publish(self._mved_destance)
